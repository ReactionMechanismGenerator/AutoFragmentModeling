    ...
    for match in matches:
        ...
        if r_frag_1 not in rr_list:
            if r_frag_2 not in rr_list:
                ...
                new_matches.append(((r_frag_1, r_frag_2, r_frag_1), value / 2.0))
        else:
            if r_frag_2 not in rr_list:
                # cases like (L-W1-L, X-R)
                new_matches.append(((r_frag_2, r_frag_1, r_frag_2), value / 2.0))
            else:
                # cases like (L-W1-L, R-U1-R)
                new_r_moles.append((pair, value / 2.0))

    return new_matches, new_r_moles
